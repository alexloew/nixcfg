...
let
  ...
  wallpaperScript = pkgs.writeShellScript "set-wallpapers" ''
    ...
    args=()
    [ -n "$uw"  ] && args+=(--output "$uw"  --image "$wp/earthrise.JPG" --mode fill)
    [ -n "$aw"  ] && args+=(--output "$aw"  --image "$wp/earthrise.JPG" --mode fill)
    [ -n "$edp" ] && args+=(--output "$edp" --image "$wp/earthrise.JPG" --mode fill)
